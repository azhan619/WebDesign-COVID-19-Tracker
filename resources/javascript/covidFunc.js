const covidApi = {
    getSummary: async () => {
        return await fData(covidApiEndPoints.summary())
    },
    getWorldAllTimeCases: async () => {
        return await fData(covidApiEndPoints.worldAllTimeCases())
    },
    getCountryAllTimeCases: async (country, status) => {
        return await fData(covidApiEndPoints.countryAllTimeCases(country, status))
    },
    getWorldDaysCases: async () => {
        return await fData(covidApiEndPoints.worldDaysCases())
    },
    getCountryDaysCases: async (country, status) => {
        return await fData(covidApiEndPoints.countryDaysCases(country, status))
    }
}
